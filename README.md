...
使用 **LLM as Judge** 方式（DeepSeek 当裁判），从三个维度评估回答质量：

| 指标 | 说明 | 结果 |
|------|------|------|
| Faithfulness（忠实度） | 回答是否基于文档、不编造 | 1.00 |
| Relevancy（相关性） | 回答是否切题 | 1.00 |
| Recall（召回率） | 是否覆盖关键信息 | 1.00 |

> K=1 / K=4 / K=8 对比实验见 `evaluate_compare.py`
